...
impl SystemTray {
	...
	/// Sets the tray icon. Must be a [`Icon::File`] on Linux and a
	/// [`Icon::Raw`] on Windows and macOS.
	#[must_use]
...
pub trait RuntimeHandle: Debug + Clone + Send + Sync + Sized + 'static {
	...
	fn create_window(&self, pending: PendingWindow<Self::Runtime>) -> crate::Result<DetachedWindow<Self::Runtime>>;
...
pub trait Dispatch: Debug + Clone + Send + Sync + Sized + 'static {
	...
	/// Create a new webview window.
	fn create_window(&mut self, pending: PendingWindow<Self::Runtime>) -> crate::Result<DetachedWindow<Self::Runtime>>;
