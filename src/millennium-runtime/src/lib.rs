...
impl SystemTray {
	...
	/// Sets the tray icon. Must be a [`TrayIcon::File`] on Linux and a
...
impl TrayIcon {
	...
	/// We expect the code that passes the Icon enum to have already checked the
	/// platform.
	#[cfg(target_os = "linux")]
...
pub trait RuntimeHandle<T: UserEvent>: Debug + Clone + Send + Sync + Sized + 'static {
	...
	fn create_window(&self, pending: PendingWindow<T, Self::Runtime>) -> crate::Result<DetachedWindow<T, Self::Runtime>>;
...
pub trait GlobalShortcutManager: Debug + Clone + Send + Sync {
	...
	fn register<F: Fn() + Send + 'static>(&mut self, accelerator: &str, handler: F) -> crate::Result<()>;
...
pub trait Runtime<T: UserEvent>: Debug + Sized + 'static {
	...
	/// Create a new webview window.
	fn create_window(&self, pending: PendingWindow<T, Self>) -> crate::Result<DetachedWindow<T, Self>>;
...
pub trait Dispatch<T: UserEvent>: Debug + Clone + Send + Sync + Sized + 'static {
	...
	/// Create a new webview window.
	fn create_window(&mut self, pending: PendingWindow<T, Self::Runtime>) -> crate::Result<DetachedWindow<T, Self::Runtime>>;
