...
impl Dispatch for MillenniumDispatcher {
	...
	fn create_window(&mut self, pending: PendingWindow<Self::Runtime>) -> Result<DetachedWindow<Self::Runtime>> {
		...
		send_user_message(
			&self.context,
			Message::CreateWebview(Box::new(move |event_loop, web_context| create_webview(event_loop, web_context, context, pending)), tx)
		)?;
...
impl Runtime for MillenniumWebview {
	...
	fn create_window(&self, pending: PendingWindow<Self>) -> Result<DetachedWindow<Self>> {
		...
		#[cfg(all(target_os = "windows", not(feature = "__rust_analyzer_hack")))]
		{
			...
			if let WindowHandle::Webview(ref webview) = webview.inner {
				...
				unsafe {
					controller.GotFocus(
						FocusChangedEventHandler::create(Box::new(move |_, _| {
							let _ = proxy.send_event(Message::Webview(id, WebviewMessage::WebviewEvent(WebviewEvent::Focused(true))));
							Ok(())
						})),
						&mut token
					)
				...
				unsafe {
					controller.LostFocus(
						FocusChangedEventHandler::create(Box::new(move |_, _| {
							let _ = proxy.send_event(Message::Webview(id, WebviewMessage::WebviewEvent(WebviewEvent::Focused(false))));
							Ok(())
						})),
						&mut token
					)
		...
		self.windows.lock().unwrap().insert(webview.inner.window().id(), webview);
...
fn create_webview(event_loop: &EventLoopWindowTarget<Message>, web_context: &WebContextStore, context: Context, pending: PendingWindow<MillenniumWebview>) -> Result<WindowWrapper> {
	...
	if let Some(handler) = ipc_handler {
		webview_builder = webview_builder.with_ipc_handler(create_ipc_handler(context.clone(), label.clone(), menu_ids.clone(), js_event_listeners.clone(), handler));
	}
	if let Some(handler) = file_drop_handler {
		webview_builder = webview_builder.with_file_drop_handler(create_file_drop_handler(context, label.clone(), menu_ids, js_event_listeners, handler));
	}
...
/// Create a Millennium Webview file drop handler from a Millennium file drop
/// handler.
fn create_file_drop_handler(
	...
	menu_ids: Arc<Mutex<HashMap<MenuHash, MenuId>>>,
	js_event_listeners: Arc<Mutex<HashMap<JsEventListenerKey, HashSet<u64>>>>,
	handler: FileDropHandler<MillenniumWebview>
) -> Box<dyn Fn(&Window, MillenniumFileDropEvent) -> bool + 'static> {
