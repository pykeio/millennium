...
impl WindowManagerCmd {
	fn into_allowlist_error(self) -> crate::Error {
		match self {
			...
			Self::RequestUserAttention(_) => crate::Error::ApiNotAllowlisted("window > requestUserAttention".to_string()),
			...
			Self::ToggleMaximize => crate::Error::ApiNotAllowlisted("window > maximize and window > unmaximize".to_string()),
			...
			Self::SetDecorations(_) => crate::Error::ApiNotAllowlisted("window > setDecorations".to_string()),
			Self::SetAlwaysOnTop(_) => crate::Error::ApiNotAllowlisted("window > setAlwaysOnTop".to_string()),
	...
	async fn create_webview<R: Runtime>(context: InvokeContext<R>, options: Box<WindowConfig>) -> super::Result<()> {
		...
		window
			.create_window(label, url, |_, webview_attributes| {
				(<<R::Dispatcher as Dispatch>::WindowBuilder>::with_config(*options), webview_attributes)
			})
