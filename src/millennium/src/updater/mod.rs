...
impl<R: Runtime> UpdateBuilder<R> {
	...
	/// 		let builder = millennium::updater::builder(handle).target(format!("{}-{}", millennium::updater::target().unwrap(), kind));
	/// 		match builder.check().await {
	/// 			Ok(update) => {}
	/// 			Err(error) => {}
	/// 		}
	...
	/// 	let handle = app.handle();
	/// 	millennium::async_runtime::spawn(async move {
	/// 		let builder = millennium::updater::builder(handle).target(millennium::utils::platform::target_triple().unwrap());
	/// 		match builder.check().await {
	/// 			Ok(update) => {}
	/// 			Err(error) => {}
	/// 		}
