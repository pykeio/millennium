...
impl<R: Runtime> App<R> {
	...
	fn run_updater_dialog(&self, window: Window<R>) {
		...
		crate::async_runtime::spawn(async move { updater::check_update_with_dialog(updater_config, package_info, window).await });
	}
	...
	fn run_updater(&self, main_window: Option<Window<R>>) {
		if let Some(main_window) = main_window {
			...
			if updater_config.dialog && updater_config.active {
				...
				// invoke the Event `millennium://update` from JS or rust side.
				main_window.listen(updater::EVENT_CHECK_UPDATE, move |_msg| {
					...
					crate::async_runtime::spawn(async move { updater::check_update_with_dialog(config, package_info, window).await });
				});
			} else if updater_config.active {
				// we only listen for `millennium://update`
				// once we receive the call, we check if an update is available or not
				// if there is a new update we emit `millennium://update-available` with details
				// this is the user responsabilities to display dialog and ask if user want to
				// install to install the update you need to invoke the Event
