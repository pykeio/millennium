...
///   .setup(|app| {
///     let path = millennium::api::path::parse(&app.config(), app.package_info(), &app.env(), "$HOME/.bashrc").unwrap();
...
///
/// ```rust,no_run
/// use millennium::{api::path::{BaseDirectory, resolve_path}, Env};
...
/// ```rust,no_run
/// use millennium::{api::path::{BaseDirectory, resolve_path}, Manager};
/// millennium::Builder::default()
///   .setup(|app| {
///     let path = resolve_path(
///       &app.config(),
