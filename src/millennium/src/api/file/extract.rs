...
impl<R: Read + Seek> Extract<R> {
	...
	fn get_archive_reader(source: &mut R, compression: Option<Compression>) -> Either<&mut R, flate2::read::GzDecoder<&mut R>> {
	...
	/// If it's a directory, the target will be created, if it's a file, it'll
	/// be extracted at this location. Note: You need to include the complete
	/// path, with file name and extension.
	pub fn extract_file<T: AsRef<path::Path>>(&mut self, into_path: &path::Path, file_to_extract: T) -> crate::api::Result<()> {
		...
		match self.archive_format {
				...
				match self.archive_format {
					...
					ArchiveFormat::Tar(_) => {
						...
						let mut entry = archive
							...
							.ok_or_else(|| crate::api::Error::Extract(format!("Could not find the required path in the archive: {:?}", file_to_extract)))?;
			...
			ArchiveFormat::Zip => {
				let mut archive = zip::ZipArchive::new(reader)?;
				let mut file = archive.by_name(file_to_extract.to_str().expect("Could not convert file to str"))?;
