...
impl<R: Runtime> Window<R> {
	...
	pub fn builder<M: Manager<R>, L: Into<String>>(manager: &M, label: L, url: WindowUrl) -> WindowBuilder<R> {
	...
	pub fn hwnd(&self) -> crate::Result<*mut std::ffi::c_void> {
		self.window.dispatcher.hwnd().map(|hwnd| hwnd.0 as *mut _).map_err(Into::into)
	}
