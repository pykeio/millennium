...
use windows::{
	...
	Win32::{
		...
		UI::WindowsAndMessaging::{self as win32wm, DestroyWindow, GetClientRect, GetCursorPos, WM_NCLBUTTONDOWN}
	}
};
		...
		CreateCoreWebView2EnvironmentCompletedHandler::wait_for_async_operation(
			Box::new(move |environmentcreatedhandler| unsafe {
				if let Some(data_directory) = data_directory {
					// If we have a custom data_directory, we need to use a call to
					// `CreateCoreWebView2EnvironmentWithOptions` instead of the much simpler
				...
				CreateCoreWebView2Environment(environmentcreatedhandler).map_err(webview2_com::Error::WindowsError)
			}),
...
window.addEventListener('mousemove', (e) => window.chrome.webview.postMessage('__WEBVIEW_MOUSE_MOVE__'));"#
							...
							if let Some(args) = args {
								...
								if let Ok(content) = webview_request.Content() {
									...
									loop {
										...
										content.Read(buffer.as_mut_ptr() as *mut _, buffer.len() as u32, &mut cb_read)?;
									...
									return match (custom_protocol.1)(&final_request) {
										Ok(sent_response) => {
											...
											if !content.is_empty() {
												...
												if stream.Write(content.as_ptr() as *const _, content.len() as u32)? as usize == content.len() {
	...
	pub fn resize(&self, hwnd: HWND) -> Result<()> {
		// Safety: System calls are unsafe
		// XXX: Resizing on Windows is usually sluggish. Many other applications share
		// same behavior.
		unsafe {
			let mut rect = RECT::default();
			GetClientRect(hwnd, &mut rect);
...
pub fn platform_webview_version() -> Result<String> {
	...
	unsafe { GetAvailableCoreWebView2BrowserVersionString(PWSTR::default(), &mut versioninfo) }.map_err(webview2_com::Error::WindowsError)?;
