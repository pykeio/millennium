...
//!
//! - `file-drop`: Enables [`with_file_drop_handler`] to control the behaviour
//!   when there are files
//! interacting with the window. Enabled by default.
//! - `protocol`: Enables [`with_custom_protocol`] to define custom URL scheme
//!   for handling tasks like
//! loading assets. Enabled by default.
//! - `tray`: Enables system tray and more menu item variants on **Linux**. You
//!   can still create
...
//! by default.
//! - `ayatana`: Enable this if you wish to use more update
//!   `libayatana-appindicator` since
...
//! Avoid this in release build if your app needs to publish to App Store.
//! - `dox`: Enables this in `package.metadata.docs.rs` section to skip linking
//!   some **Linux**
