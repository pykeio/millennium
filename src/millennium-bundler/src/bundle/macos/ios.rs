...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
	...
	let bundle_dir = settings.project_out_directory().join("bundle/ios").join(&app_product_name);
	if bundle_dir.exists() {
		fs::remove_dir_all(&bundle_dir).with_context(|| format!("Failed to remove old {}", app_product_name))?;
	}
	fs::create_dir_all(&bundle_dir).with_context(|| format!("Failed to create bundle directory at {:?}", bundle_dir))?;

	for src in settings.resource_files() {
		let src = src?;
		let dest = bundle_dir.join(millennium_utils::resources::resource_relpath(&src));
		common::copy_file(&src, &dest).with_context(|| format!("Failed to copy resource file {:?}", src))?;
	}

	let icon_filenames = generate_icon_files(&bundle_dir, settings).with_context(|| "Failed to create app icons")?;
	generate_info_plist(&bundle_dir, settings, &icon_filenames).with_context(|| "Failed to create Info.plist")?;

	for bin in settings.binaries() {
		let bin_path = settings.binary_path(bin);
		common::copy_file(&bin_path, &bundle_dir.join(bin.name())).with_context(|| format!("Failed to copy binary from {:?}", bin_path))?;
	}
		...
		for icon_path in settings.icon_files() {
			...
			if icon_path.extension() == Some(OsStr::new("png")) {
				...
				if !sizes.contains(&(width, height, is_retina)) {
					sizes.insert((width, height, is_retina));
					let dest_path = get_dest_path(width, height, is_retina);
					icon.write_to(&mut common::create_file(&dest_path)?, image::ImageOutputFormat::Png)?;
				}
			}
		}
	}
...
fn generate_info_plist(bundle_dir: &Path, settings: &Settings, icon_filenames: &[String]) -> crate::Result<()> {
	...
	writeln!(file, "  <key>CFBundleIdentifier</key>\n  <string>{}</string>", settings.bundle_identifier())?;
	writeln!(file, "  <key>CFBundleDisplayName</key>\n  <string>{}</string>", settings.product_name())?;
	writeln!(file, "  <key>CFBundleName</key>\n  <string>{}</string>", settings.product_name())?;
	writeln!(file, "  <key>CFBundleExecutable</key>\n  <string>{}</string>", settings.main_binary_name())?;
	writeln!(file, "  <key>CFBundleVersion</key>\n  <string>{}</string>", settings.version_string())?;
	writeln!(file, "  <key>CFBundleShortVersionString</key>\n  <string>{}</string>", settings.version_string())?;
