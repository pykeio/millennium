...
pub fn generate_data(settings: &Settings, package_dir: &Path) -> crate::Result<(PathBuf, BTreeSet<DebIcon>)> {
	...
	if use_bootstrapper {
		generate_bootstrap_file(settings, &data_dir).with_context(|| "Failed to generate bootstrap file")?;
	}
...
fn generate_desktop_file(settings: &Settings, data_dir: &Path) -> crate::Result<()> {
	...
	let use_bootstrapper = settings.deb().use_bootstrapper.unwrap_or_default();
	writeln!(file, "Exec={}", if use_bootstrapper { format!("__{}-bootstrapper", bin_name) } else { bin_name.to_string() })?;
...
fn generate_icon_files(settings: &Settings, data_dir: &Path) -> crate::Result<BTreeSet<DebIcon>> {
	...
	for icon_path in settings.icon_files() {
		...
		if icon_path.extension() == Some(OsStr::new("png")) {
			...
			if !icons.contains(&deb_icon) {
				icon.write_to(&mut common::create_file(&deb_icon.path)?, image::ImageOutputFormat::Png)?;
				icons.insert(deb_icon);
			}
		}
	}
