...
pub fn command(options: Options) -> Result<()> {
	...
	let mut watcher = watcher(tx, Duration::from_secs(1)).unwrap();
	watcher.watch(millennium_path.join("src"), RecursiveMode::Recursive)?;
	watcher.watch(millennium_path.join("Cargo.toml"), RecursiveMode::Recursive)?;
	watcher.watch(millennium_path.join(".millenniumrc"), RecursiveMode::Recursive)?;

	for member in get_workspace_members()? {
		let workspace_path = millennium_path.join(member);
		...
		if let Ok(event) = rx.recv() {
			...
			if let Some(event_path) = event_path {
				if event_path.file_name() == Some(OsStr::new(".millenniumrc")) {
					...
					process.kill().with_context(|| "failed to kill app process")?;
