...
pub fn does_supported_extension_exist(path: impl Into<PathBuf>) -> bool {
	let path = path.into();
	path.exists() || EXTENSIONS_ENABLED.iter().any(|ext| path.with_extension(ext).exists())
}
...
fn do_parse<D: DeserializeOwned>(path: PathBuf) -> Result<D, ConfigError> {
	...
	if path.exists() {
		...
		// will be returned. this could be a bit confusing, so we may want to encourage
		// users using json5 to use the .json5 extension instead of .json
		#[cfg(feature = "config-json5")]
		let json = {
