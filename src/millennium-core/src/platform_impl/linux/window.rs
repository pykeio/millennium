...
impl Window {
	...
	pub fn set_min_inner_size<S: Into<Size>>(&self, min_size: Option<S>) {
		if let Some(size) = min_size {
			...
			if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::MinSize((min_width, min_height)))) {
	...
	pub fn set_max_inner_size<S: Into<Size>>(&self, max_size: Option<S>) {
		if let Some(size) = max_size {
			...
			if let Err(e) = self.window_requests_tx.send((self.window_id, WindowRequest::MaxSize((max_width, max_height)))) {
