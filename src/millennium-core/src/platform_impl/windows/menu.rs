...
impl Menu {
	...
	pub fn add_item(
		...
		unsafe {
			...
			AppendMenuW(self.hmenu, flags, menu_id.0 as _, PWSTR(util::encode_wide(title).as_mut_ptr()));
...
fn execute_edit_command(command: EditCommand) {
	...
	unsafe {
		...
		SendInput(inputs.len() as _, inputs.as_mut_ptr(), std::mem::size_of::<INPUT>() as _);
	}
}
