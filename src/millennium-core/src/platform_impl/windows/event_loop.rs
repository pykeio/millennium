...
use windows::Win32::{
	...
	Foundation::{BOOL, HANDLE, HINSTANCE, HWND, LPARAM, LRESULT, POINT, PWSTR, RECT, WAIT_TIMEOUT, WPARAM},
	Graphics::Gdi::*,
	System::{
...
lazy_static! {
	...
	static ref THREAD_EVENT_TARGET_WINDOW_CLASS: Vec<u16> = unsafe {
		let mut class_name= util::to_wstring("MillenniumCore Thread Event Target");
...
unsafe fn public_window_callback_inner<T: 'static>(
	...
	let keyboard_callback = || {
		...
		let events = {
			let mut window_state = subclass_input.window_state.lock();
			window_state.key_event_builder.process_message(window, msg, wparam, lparam, &mut result)
		};
		for event in events {
			subclass_input.send_event(Event::WindowEvent {
				window_id: RootWindowId(WindowId(window.0)),
