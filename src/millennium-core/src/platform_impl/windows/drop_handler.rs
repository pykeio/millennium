...
impl FileDropHandler {
	...
	unsafe fn DragEnter(&mut self, pDataObj: &Option<IDataObject>, _grfKeyState: u32, _pt: POINTL, pdwEffect: *mut u32) -> windows::core::Result<()> {
		...
		self.cursor_effect = if self.hovered_is_valid { DROPEFFECT_COPY } else { DROPEFFECT_NONE };
		*pdwEffect = self.cursor_effect;
		Ok(())
	}

	unsafe fn DragOver(&self, _grfKeyState: u32, _pt: POINTL, pdwEffect: *mut u32) -> windows::core::Result<()> {
		*pdwEffect = self.cursor_effect;
		Ok(())
	}
	...
	unsafe fn Drop(&self, pDataObj: &Option<IDataObject>, _grfKeyState: u32, _pt: POINTL, _pdwEffect: *mut u32) -> windows::core::Result<()> {
