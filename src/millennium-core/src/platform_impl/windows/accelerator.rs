...
// unless the MSDN Library article for the function explicitly mentions it is
// not.
...
lazy_static! {
	static ref ACCEL_TABLES: Mutex<HashMap<WindowHandle, Arc<AccelTable>>> = Mutex::new(HashMap::default());
}
...
impl AccelTable {
	fn new(accel: &[ACCEL]) -> AccelTable {
		...
		AccelTable { accel: AccelHandle(accel.0) }
	}
