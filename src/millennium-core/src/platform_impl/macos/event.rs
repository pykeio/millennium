...
pub fn create_key_event(ns_event: id, is_press: bool, is_repeat: bool, in_ime: bool, key_override: Option<KeyCode>) -> KeyEvent {
	...
	let text = if in_ime || !is_press { None } else { logical_key.to_text() };
	KeyEvent {
...
pub fn get_scancode(event: cocoa::base::id) -> c_ushort {
	// In AppKit, `keyCode` refers to the position (scancode) of a key rather than
	// its character, and there is no easy way to navtively retrieve the
	// layout-dependent character. In Millennium Core, we use keycode to refer to
	// the key's character, and so this function aligns AppKit's terminology with
	// ours.
