	...
	fn encode() -> Encoding {
		unsafe {
			if cfg!(target_pointer_width = "32") {
				return Encoding::from_str("{CGRect={CGPoint=ff}{CGSize=ff}}");
			} else if cfg!(target_pointer_width = "64") {
				return Encoding::from_str("{CGRect={CGPoint=dd}{CGSize=dd}}");
			} else {
				unimplemented!();
			}
		}
...
impl BitOr for UIInterfaceOrientationMask {
	...
	fn bitor(self, rhs: Self) -> Self {
		return UIInterfaceOrientationMask(self.0 | rhs.0);
	}
}

impl UIInterfaceOrientationMask {
	pub fn from_valid_orientations_idiom(valid_orientations: ValidOrientations, idiom: Idiom) -> UIInterfaceOrientationMask {
		return match (valid_orientations, idiom) {
...
impl From<ScreenEdge> for UIRectEdge {
	fn from(screen_edge: ScreenEdge) -> UIRectEdge {
		...
		return UIRectEdge(screen_edge.bits().into());
	}
}
...
impl Into<ScreenEdge> for UIRectEdge {
	fn into(self) -> ScreenEdge {
		...
		return ScreenEdge::from_bits(bits).expect("invalid `ScreenEdge`");
	}
}
...
extern "C" {
	...
	pub fn UIApplicationMain(argc: c_int, argv: *const c_char, principalClassName: id, delegateClassName: id) -> c_int;
...
// This is named NSStringRust rather than NSString because the "Debug View
// Heirarchy" feature of Xcode requires a non-ambiguous reference to NSString
// for unclear reasons. This makes Xcode happy so please test if you change the
// name back to NSString.
pub trait NSStringRust: Sized {
	unsafe fn alloc(_: Self) -> id {
		return msg_send![class!(NSString), alloc];
	}
...
impl NSStringRust for id {
	unsafe fn initWithUTF8String_(self, c_string: *const c_char) -> id {
		return msg_send![self, initWithUTF8String: c_string as id];
	}

	unsafe fn stringByAppendingString_(self, other: id) -> id {
		return msg_send![self, stringByAppendingString: other];
	}

	unsafe fn init_str(self, string: &str) -> id {
		let cstring = CString::new(string).unwrap();
		return self.initWithUTF8String_(cstring.as_ptr());
	}

	unsafe fn UTF8String(self) -> *const c_char {
		return msg_send![self, UTF8String];
	}
}
