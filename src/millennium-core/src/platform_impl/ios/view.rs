...
use crate::{
	...
	platform_impl::platform::{
		...
		ffi::{id, nil, CGFloat, CGPoint, CGRect, UIForceTouchCapability, UIInterfaceOrientationMask, UIRectEdge, UITouchPhase, UITouchType},
...
macro_rules! add_property {
	...
	(
	...
	) => {
		{
			...
			$decl.add_method(sel!($setter_name:), setter as extern "C" fn(&mut Object, Sel, $t));
			$decl.add_method(sel!($getter_name), $getter_name as extern "C" fn(&Object, Sel) -> $t);
		}
	};
}
...
unsafe fn get_view_class(root_view_class: &'static Class) -> &'static Class {
	...
	return classes.entry(root_view_class).or_insert_with(move || {
		...
		extern "C" fn layout_subviews(object: &Object, _: Sel) {
			unsafe {
				...
				let screen_frame: CGRect = msg_send![object, convertRect:window_bounds toCoordinateSpace:screen_space];
				...
				// If the app is started in landscape, the view frame and window bounds can be
				// mismatched. The view frame will be in portrait and the window bounds in
		...
		extern "C" fn set_content_scale_factor(object: &mut Object, _: Sel, untrusted_scale_factor: CGFloat) {
			unsafe {
				let superclass: &'static Class = msg_send![object, superclass];
				let () = msg_send![super(object, superclass), setContentScaleFactor: untrusted_scale_factor];
				...
				// UIKit when the `UIView` is first created), in which case we send no events
				// here
				if window.is_null() {
					return;
				}

				// `setContentScaleFactor` may be called with a value of 0, which means "reset
				// the content scale factor to a device-specific default value", so we can't use
				// the parameter here. We can query the actual factor using the getter
				let scale_factor: CGFloat = msg_send![object, contentScaleFactor];
				assert!(
					!scale_factor.is_nan() && scale_factor.is_finite() && scale_factor.is_sign_positive() && scale_factor > 0.0,
			...
			unsafe {
				...
				loop {
					...
					let force = if os_supports_force {
						...
						let touch_capability: UIForceTouchCapability = msg_send![trait_collection, forceTouchCapability];
					...
					let physical_location = {
						let scale_factor: CGFloat = msg_send![object, contentScaleFactor];
						PhysicalPosition::from_logical::<(f64, f64), f64>((logical_location.x as _, logical_location.y as _), scale_factor)
					};
		...
		let mut decl = ClassDecl::new(&format!("MillenniumUIView{}", ID), root_view_class).expect("Failed to declare class `MillenniumUIView`");
...
unsafe fn get_view_controller_class() -> &'static Class {
	...
	if CLASS.is_none() {
		...
		let mut decl = ClassDecl::new("MillenniumUIViewController", uiviewcontroller_class).expect("Failed to declare class `MillenniumUIViewController`");
		decl.add_method(sel!(shouldAutorotate), should_autorotate as extern "C" fn(&Object, Sel) -> BOOL);
		add_property! {
			decl,
...
unsafe fn get_window_class() -> &'static Class {
	...
	if CLASS.is_none() {
		...
		let mut decl = ClassDecl::new("MillenniumUIWindow", uiwindow_class).expect("Failed to declare class `MillenniumUIWindow`");
...
// requires main thread
pub unsafe fn create_view(_window_attributes: &WindowAttributes, platform_attributes: &PlatformSpecificWindowBuilderAttributes, frame: CGRect) -> id {
...
// requires main thread
pub unsafe fn create_view_controller(_window_attributes: &WindowAttributes, platform_attributes: &PlatformSpecificWindowBuilderAttributes, view: id) -> id {
	...
	let supported_orientations = UIInterfaceOrientationMask::from_valid_orientations_idiom(platform_attributes.valid_orientations, idiom);
	let prefers_home_indicator_hidden = if platform_attributes.prefers_home_indicator_hidden { YES } else { NO };

	let edges: UIRectEdge = platform_attributes.preferred_screen_edges_deferring_system_gestures.into();
	...
	let () = msg_send![view_controller, setPrefersHomeIndicatorAutoHidden: prefers_home_indicator_hidden];
	let () = msg_send![view_controller, setPreferredScreenEdgesDeferringSystemGestures: edges];
...
pub fn create_delegate_class() {
	...
	extern "C" fn will_terminate(_: &Object, _: Sel, _: id) {
		unsafe {
			...
			loop {
				...
				let is_millennium_window: BOOL = msg_send![window, isKindOfClass: class!(MillenniumUIWindow)];
				if is_millennium_window == YES {
	...
	unsafe {
		...
		decl.add_method(sel!(applicationWillEnterForeground:), will_enter_foreground as extern "C" fn(&Object, Sel, id));
		decl.add_method(sel!(applicationDidEnterBackground:), did_enter_background as extern "C" fn(&Object, Sel, id));

		decl.add_method(sel!(applicationWillTerminate:), will_terminate as extern "C" fn(&Object, Sel, id));
