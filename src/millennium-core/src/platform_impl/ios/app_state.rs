...
macro_rules! bug {
	($($msg:tt)*) => {
		panic!("iOS bug, file an issue: {}", format!($($msg)*));
	};
}

macro_rules! bug_assert {
	($test:expr, $($msg:tt)*) => {
		assert!($test, "iOS bug, file an issue: {}", format!($($msg)*));
	};
}
...
impl Drop for AppState {
	fn drop(&mut self) {
		match self.state_mut() {
			&mut AppStateImpl::NotLaunched { ref mut queued_windows, .. } | &mut AppStateImpl::Launching { ref mut queued_windows, .. } => {
...
impl AppState {
	...
	unsafe fn get_mut() -> RefMut<'static, AppState> {
		// basically everything in UIKit requires the main thread, so it's pointless to
		// use the std::sync APIs. must be mut because plain `static` requires `Sync`
		static mut APP_STATE: RefCell<Option<AppState>> = RefCell::new(None);

		if cfg!(debug_assertions) {
			assert_main_thread!("bug: `AppState::get_mut()` can only be called on the main thread");
		}
		...
		return RefMut::map(guard, |state| state.as_mut().unwrap());
	}

	fn state(&self) -> &AppStateImpl {
		return match &self.app_state {
	...
	fn state_mut(&mut self) -> &mut AppStateImpl {
		return match &mut self.app_state {
	...
	fn take_state(&mut self) -> AppStateImpl {
		return match self.app_state.take() {
			Some(state) => state,
			None => bug!("`AppState` previously failed a state transition")
		};
	}
	...
	fn wakeup_transition(&mut self) -> Option<EventWrapper> {
		// before `AppState::did_finish_launching` is called, pretend there is no
		// running event loop.
		...
		let (event_handler, event) = match (self.control_flow, self.take_state()) {
			...
			(ControlFlow::WaitUntil(requested_resume), AppStateImpl::Waiting { waiting_event_handler, start }) => {
				...
				return (waiting_event_handler, event);
			}
	...
	fn try_user_callback_transition(&mut self) -> UserCallbackTransitionResult<'_> {
		// If we're not able to process an event due to recursion or `Init` not having
		// been sent out yet, then queue the events up.
		match self.state_mut() {
		...
		let (event_handler, queued_gpu_redraws, active_control_flow, processing_redraws) = match self.take_state() {
		...
		return UserCallbackTransitionResult::Success {
			event_handler,
			active_control_flow,
			processing_redraws
		};
	...
	fn events_cleared_transition(&mut self) {
		...
		match (old, new) {
			(ControlFlow::Poll, ControlFlow::Poll) => self.set_state(AppStateImpl::PollFinished { waiting_event_handler }),
	...
	fn terminated_transition(&mut self) -> Box<dyn EventHandler> {
		return match self.replace_state(AppStateImpl::Terminated) {
			AppStateImpl::ProcessingEvents { event_handler, .. } => event_handler,
			s => bug!("`LoopDestroyed` happened while not processing events {:?}", s)
		};
	}
}
...
pub unsafe fn set_key_window(window: id) {
	...
	match this.state_mut() {
		&mut AppStateImpl::NotLaunched { ref mut queued_windows, .. } => return queued_windows.push(msg_send![window, retain]),
...
pub unsafe fn did_finish_launching() {
	...
	bug_assert!(this.control_flow == ControlFlow::Poll, "unexpectedly not setup to `Poll` on launch!");
	this.waker.start();

	// have to drop RefMut because the window setup code below can trigger new
	// events
	drop(this);
	...
	// created. we can just set those windows up normally, as they were created
...
pub unsafe fn handle_nonuser_events<I: IntoIterator<Item = EventWrapper>>(events: I) {
	let mut this = AppState::get_mut();
	let (mut event_handler, active_control_flow, processing_redraws) = match this.try_user_callback_transition() {
		...
		match wrapper {
			EventWrapper::StaticEvent(event) => {
				if !processing_redraws && event.is_redraw() {
					...
					log::warn!("processing non `RedrawRequested` event after the main event loop: {:#?}", event);
				}
...
fn handle_event_proxy(event_handler: &mut Box<dyn EventHandler>, control_flow: ControlFlow, proxy: EventProxy) {
	match proxy {
...
impl EventLoopWaker {
	fn new(rl: CFRunLoopRef) -> EventLoopWaker {
		...
		unsafe {
			...
			let timer = CFRunLoopTimerCreate(ptr::null_mut(), std::f64::MAX, 0.000_000_1, 0, 0, wakeup_main_loop, ptr::null_mut());
			CFRunLoopAddTimer(rl, timer, kCFRunLoopCommonModes);

			return EventLoopWaker { timer };
		}
	}
	...
	(
		...
		impl From<NSOperatingSystemVersion> for OSCapabilities {
			fn from(os_version: NSOperatingSystemVersion) -> OSCapabilities {
				...
				return OSCapabilities { $($name,)* os_version };
			}
		}
...
impl NSOperatingSystemVersion {
	fn meets_requirements(&self, required_major: NSInteger, required_minor: NSInteger) -> bool {
		return (self.major, self.minor) >= (required_major, required_minor);
	}
}
	...
	lazy_static! {
		static ref OS_CAPABILITIES: OSCapabilities = {
			let version: NSOperatingSystemVersion = unsafe {
				...
				// iOS >8 required because no one has put the time into supporting earlier os versions.
				...
				assert!(atleast_ios_8 == YES, "Millennium Core requires iOS version 8 or greater");
				return msg_send![process_info, operatingSystemVersion];
			};
			return version.into();
		};
	}
	return OS_CAPABILITIES.clone();
}
