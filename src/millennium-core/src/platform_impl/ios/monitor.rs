...
impl Clone for NativeDisplayMode {
	fn clone(&self) -> Self {
		...
		return NativeDisplayMode(self.0);
	}
}
...
impl VideoMode {
	unsafe fn retained_new(uiscreen: id, screen_mode: id) -> VideoMode {
		...
		let refresh_rate: NSInteger = if os_capabilities.maximum_frames_per_second {
			...
			//
			// All iOS devices support 60 fps, and on devices where `maximumFramesPerSecond`
			// is not supported, they are all guaranteed to have 60hz refresh rates. This
			// does not correctly handle external displays. ProMotion displays support
			// 120fps, but they were introduced at the same time as the
			// `maximumFramesPerSecond` API.
			//
	...
	pub fn monitor(&self) -> RootMonitorHandle {
		return RootMonitorHandle { inner: self.monitor.clone() };
	}
}
...
impl Clone for MonitorHandle {
	fn clone(&self) -> MonitorHandle {
		return MonitorHandle::retained_new(self.uiscreen);
	}
}
...
impl MonitorHandle {
	pub fn retained_new(uiscreen: id) -> MonitorHandle {
		...
		return MonitorHandle { inner: Inner { uiscreen } };
	}
}
...
impl Inner {
	...
	pub fn size(&self) -> PhysicalSize<u32> {
		unsafe {
			...
			return PhysicalSize::new(bounds.size.width as u32, bounds.size.height as u32);
		}
	}
	...
	pub fn position(&self) -> PhysicalPosition<i32> {
		unsafe {
			let bounds: CGRect = msg_send![self.ui_screen(), nativeBounds];
			return (bounds.origin.x as f64, bounds.origin.y as f64).into();
		}
	}
...
pub unsafe fn main_uiscreen() -> MonitorHandle {
	...
	return MonitorHandle::retained_new(uiscreen);
}
