...
pub enum ControlFlow {
	...
	/// - **Unix**: On most Unix-like platforms, only the 8 least significant
	///   bits will be used, which can cause surprises with negative exit values
	///   (`-42` would end up as `214`). See [`std::process::exit`].
	///
...
impl<T> EventLoop<T> {
	...
	/// ## Platform-specific
	///
	/// - **Unix**: The program terminates with exit code 1 if the display
	///   server disconnects.
	///
