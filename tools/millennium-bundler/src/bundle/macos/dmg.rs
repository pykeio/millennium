...
pub fn bundle_project(settings: &Settings, bundles: &[Bundle]) -> crate::Result<Vec<PathBuf>> {
	...
	if bundles.iter().filter(|bundle| bundle.package_type == MacOsBundle).count() == 0 {
	...
	fs::create_dir_all(&support_directory_path).with_context(|| format!("Failed to create output directory at {:?}", support_directory_path))?;
	...
	write(support_directory_path.join("eula-resources-template.xml"), include_str!("templates/dmg/eula-resources-template.xml"))?;
