...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
	...
	fs::create_dir_all(&app_bundle_path).with_context(|| format!("Failed to create bundle directory at {:?}", app_bundle_path))?;

	for src in settings.resource_files() {
		let src = src?;
		let dest = app_bundle_path.join(millennium_utils::resources::resource_relpath(&src));
		common::copy_file(&src, &dest).with_context(|| format!("Failed to copy resource file {:?}", src))?;
	}

	let icon_filenames = generate_icon_files(&app_bundle_path, settings).with_context(|| "Failed to create app icons")?;
	generate_info_plist(&app_bundle_path, settings, &icon_filenames).with_context(|| "Failed to create Info.plist")?;

	for bin in settings.binaries() {
		let bin_path = settings.binary_path(bin);
		common::copy_file(&bin_path, &app_bundle_path.join(bin.name())).with_context(|| format!("Failed to copy binary from {:?}", bin_path))?;
	}
