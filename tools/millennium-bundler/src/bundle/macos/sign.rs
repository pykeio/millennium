...
pub fn setup_keychain_if_needed() -> crate::Result<()> {
	match (std::env::var_os("APPLE_CERTIFICATE"), std::env::var_os("APPLE_CERTIFICATE_PASSWORD")) {
		(Some(certificate_encoded), Some(certificate_password)) => {
			...
			let key_chain_id = "millennium-build.keychain";
			let key_chain_name = "millennium-build";
...
pub fn delete_keychain_if_needed() {
	if let (Some(_cert), Some(_password)) = (std::env::var_os("APPLE_CERTIFICATE"), std::env::var_os("APPLE_CERTIFICATE_PASSWORD")) {
		let key_chain_id = "millennium-build.keychain";
