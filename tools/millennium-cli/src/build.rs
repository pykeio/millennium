...
pub fn command(options: Options) -> Result<()> {
	...
	if config_.millennium.bundle.active {
		...
		let package_types = if let Some(names) = options.bundles {
			...
			Some(types)
		} else if let Some(targets) = &config_.millennium.bundle.targets {
			let mut types = vec![];
		...
		let settings =
			crate::interface::get_bundler_settings(app_settings, options.target.clone(), &manifest, config_, &out_dir, options.verbose, package_types)
...
fn strip(path: &std::path::Path, logger: &Logger) -> crate::Result<()> {
	...
	let filesize_before = std::fs::metadata(&path).with_context(|| "failed to get executable file size")?.len();
	...
	let filesize_after = std::fs::metadata(&path).with_context(|| "failed to get executable file size")?.len();

	logger.log(format!("Binary stripped, size reduced by {}", (filesize_before - filesize_after).file_size(file_size_opts::CONVENTIONAL).unwrap(),));
